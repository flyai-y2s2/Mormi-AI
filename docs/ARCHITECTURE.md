...
시나리오를 제공한다. 화면 숫자와 검수 문구의 원장은 Spring BE이며, AI는
`park_context`를 산술 검증하고 대화 스냅샷으로 고정한다. 기본 문제 뒤에는 BE가 함께
보낸 전이 문제를 같은 기능으로 수행한다. AI는 화면에 없는 숫자나 새 문제를 만들지
않는다.
...
- BE의 `park_context`를 이용한 놀이동산 기본·전이 과제 스냅샷과 산술 검증
- 현재 스테이지와 하위 과제
...
- 놀이동산 화면 사실·문구 원장과 시나리오별 완료 사실 계약
...
- 놀이동산 `park_context`를 만들거나 수정하지 않고 Spring 응답의 화면 계약을 렌더링
...
```mermaid
flowchart LR
    A["정규화된 아이 응답"] --> B["understand\nSonnet 단일 판정 또는 결정형 판정"]
    B --> C["orchestrate\n안전 게이트·슬롯 검증·L/H 전이"]
    C --> H{"학습 상태를 바꾸지 않는\n안전한 사회적 발화?"}
    H -- "예" --> I["Haiku 대화 브리지\n짧은 연결 대사"]
    H -- "아니요" --> D["Sonnet speak\n의미 계약 안에서 자연스러운 대사 생성"]
    I --> E["구조·근거 출력 계약"]
    D --> E
    E -- "통과" --> J["검증 완료"]
    E -- "모델·계약 실패" --> K["검수 fallback"]
    K --> J
    J --> F["TurnContract"]
    F --> G[("접근 통제 DB 원자적 커밋")]
```

LangGraph에는 per-turn 임시 상태만 흐른다. 아이 원문을 LangGraph 체크포인터에 다시 저장하지 않는다. 영속 세션 상태와 평문 원문 기록은 접근이 통제된 애플리케이션 DB가 단일 기준이다.
...
분류기가 만드는 claim은 정답 계약과 값이 같다는 이유만으로 신뢰하지 않는다. 실제
아이 원문에 해당 의미의 근거가 있어야 하며, 순수한 거절·메타·장난 발화는 어떤 학습
슬롯도 변경할 수 없다. 사회적 발화와 학습 설명이 섞인 경우에만 원문에 실제로 드러난
학습 부분을 분리해 검증한다. 따라서 분류기가 `operation=addition`을 추측하더라도
아이가 덧셈이나 같은 의미를 말하지 않았다면 `verified_slots`로 승격되지 않는다.

정답이 아직 필요한 슬롯의 실제 정답과 금지 답 형태는 화자에게 보내지 않는다. 화자
후보는 문장 완결, 요청한 `dialogue_act`, 미해결 슬롯, 허용 사실과 원문 인용 범위를
구조적으로 검사한다. 특정 낱말이 있다는 이유만으로 자연스러운 발화를 차단하는 광범위한
정규표현식 검증이나 글자 수 강제 절단은 사용하지 않는다. 모델 호출 실패 또는 명백한
계약 위반 때만 검수된 fallback을 사용한다.
...
신뢰 경계를 넘지 않는다. 별도의 화자 재검증 LLM은 호출하지 않는다.
...
문장을 요약하지 않고 현재 `StepDefinition.prompt`와 입력 슬롯에서 생성하므로 도움
