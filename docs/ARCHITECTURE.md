...
단계가 독립 대화이므로 앞 단계의 결과는 세션 상태로 이어지지 않는다. 프론트가
다음 대화를 열 때 `cafe_context`로 넘긴다(`child_menu_id`, `paid_amount`).
`cafe_change`는 이 값들이 없으면 시작을 거부한다. 없는 값을 기본값으로 메우면
아이가 화면에서 본 것과 다른 문제를 채점하게 된다.
...
- 카페 방문 진행도 원장(어느 단계까지 왔는지, 고른 메뉴, 낸 돈)
- **결제 단계 채점**: 화폐 조합의 합계는 정오가 완전히 결정론적이므로 일반
  학습 규칙으로 두고 AI 판단과 섞지 않는다. 그 결과는 `paid_amount`로
  이 서비스에 전달된다.

줄 서기·메뉴 선택·거스름돈의 정오 판정은 이 서비스가 소유한다. 백엔드는 그
