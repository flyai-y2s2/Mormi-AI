...
카페는 5개 독립 시나리오로 시작할 수 있다. 5단계 통합 실습만 내부적으로
줄 서기→예산 메뉴 선택→직접 합계→10,000원 거스름돈을 연속 진행한다.
...
줄 인원·예산·모르미 메뉴 같은 무작위 값은 세션 생성 시 한 번 정해
`SessionState.scenario_data`에 저장한다. 턴 처리 중 다시 추첨하지 않으므로 새로고침,
서버리스 복구와 같은 `response_id` 재전송에서도 문제가 바뀌지 않는다.

예산 메뉴 고르기의 합계는 결정형 코드가 계산해 화면에 보여준다. 통합 실습에서는
합계를 아이가 먼저 계산하고, 합계가 예산보다 크다고 확인된 경우에만 메뉴 선택으로
돌아간다. 이때 아이를 오답으로 평가하지 않고 시스템의 예산 상태를 보여준다.
