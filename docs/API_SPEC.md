...
36개 집 콘텐츠는 `wrong_guess`, `incomplete_attempt`, `genuine_question` 중 검수된
진입 유형을 사용합니다. 오개념 확인형에서 아이가 “아니야”라고만 하면 같은 문제의
L4 본 질문이 이어집니다. 이 말은 답이나 이유 슬롯을 채우지 않습니다. 첫 응답에
정정한 답과 이유가 함께 있으면 한 턴에 완료될 수 있고, 답만 있으면 L4를 유지한 채
빠진 이유만 다시 묻습니다. 이 내부 단계는 별도 프론트 분기 없이 다음
`TurnContract.input`을 그대로 렌더링하면 됩니다.
