...
| `cafe` | `cafe_change` | 4단계: 낸 돈에서 두 메뉴의 합계를 빼기 |
...
카페 각 단계는 **독립된 대화**입니다. 앞 단계에서 확정된 사실은 세션 상태로
이어지지 않으므로 프론트가 다음 대화를 열 때 `cafe_context`로 함께 보냅니다.

| 필드 | 확정 주체 | 필요한 시나리오 |
|---|---|---|
| `menu_items`, `mormi_menu_id` | 프론트 | 메뉴 시나리오 전체 |
| `budget` | 프론트 | `cafe_budget_menu` |
| `child_menu_id` | 2단계에서 아이가 선택 | `cafe_change` |
| `paid_amount` | 3단계 결제, Spring 백엔드가 채점 | `cafe_change` |

화폐 조합으로 돈을 내는 결제 단계는 정오가 완전히 결정론적이므로 Spring
백엔드가 채점하고, 이 서비스는 그 결과인 `paid_amount`만 받습니다.
...
- 거스름돈은 `paid_amount − (모르미 메뉴 + 아이 메뉴)`입니다. Spring 백엔드의
  `paidAmount − orderTotal`과 같은 식이며, 내는 돈을 10,000원으로 가정하지
  않습니다.
- `cafe_change`는 `child_menu_id`와 `paid_amount`가 없으면 422로 거부합니다.
  값을 추측해 다른 문제를 내는 것보다 실패하는 편이 안전합니다.
