...
from .dataset import LADDER_ORDER
from .metrics import LadderMetrics, evaluate_label_ids
from .training_data import ID_TO_LABEL, LABEL_TO_ID
...
def load_labeled_jsonl(path: Path) -> list[dict[str, object]]:
    rows: list[dict[str, object]] = []
    expected_labels = {level.value: index for level, index in LABEL_TO_ID.items()}
    with path.open(encoding="utf-8") as handle:
        for line_number, line in enumerate(handle, start=1):
            ...
            if not isinstance(label, int) or label not in ID_TO_LABEL:
                raise ValueError(f"{path}:{line_number} has an invalid label")
            if target not in expected_labels or expected_labels[str(target)] != label:
...
def write_confusion_csv(path: Path, matrix: list[list[int]]) -> None:
    labels = [level.value for level in LADDER_ORDER]
    if len(matrix) != len(labels) or any(len(row) != len(labels) for row in matrix):
        raise ValueError("confusion matrix must be 4x4")
...
def train_model(config: TrainConfig) -> LadderMetrics:
    """Fine-tune a Korean encoder and save test metrics.

    Heavy ML imports stay inside this function so dataset preparation and
    ...
    except ImportError as exc:  # pragma: no cover - exercised by the local training environment
    ...
    model = AutoModelForSequenceClassification.from_pretrained(
        ...
        label2id={value: key for key, value in ID_TO_LABEL.items()},
    )

    def compute_metrics(prediction: Any) -> dict[str, float]:
        predicted = np.asarray(prediction.predictions).argmax(axis=-1).tolist()
        expected = np.asarray(prediction.label_ids).tolist()
        return _metric_dict(evaluate_label_ids(expected=expected, predicted=predicted))
    ...
    metrics = evaluate_label_ids(expected=expected, predicted=predicted)
    (config.output_dir / "test-metrics.json").write_text(
        json.dumps(metrics.model_dump(mode="json"), ensure_ascii=False, indent=2) + "\n",
        encoding="utf-8",
    )
    write_confusion_csv(config.output_dir / "confusion-matrix.csv", metrics.confusion_matrix)
    ...
    manifest = {
        "schema_version": 1,
        "model_version": "ladder-klue-prototype-v1",
        "base_model": config.base_model,
        "label_order": [level.value for level in LADDER_ORDER],
        ...
        "limitations": "Synthetic prototype data; not evidence of generalization to real children.",
    }
