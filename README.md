...
- 검증된 사실·미해결 슬롯·허용 근거만 화자에게 전달하는 코드 신뢰 경계
...
- **단일 의미 판정**: 자유 발화는 Sonnet 분류기가 최근 대화 맥락과 현재 질문을 함께
  보고 구조화합니다. 별도의 재판정 LLM을 호출하지 않으며, 코드가 원문 근거 없는
  claim과 사회적 발화에서 생긴 학습 claim을 차단합니다.
